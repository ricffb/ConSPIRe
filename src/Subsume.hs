...
instance forall a .(Eq a, Show a) => Subsume (Type' a)  where
  -- (<|) t1 t2 | trace (show t1 ++ "<|" ++ show t2 ++ "\n") False = undefined
  ty <| ty' | ty == ty' = True
  ind1@(TInd var1 ty1) <| ind2@(TInd var2 ty2)
    | trace (show ind1 ++ "<|" ++ show ind2 ++ "\n") False = undefined  
    | var1 == var2 = ty1 <| ty2
    | otherwise = ty1 <| traceShowId (rename var2 var1 ty2) -- Check if two inductive types only differ in their names 
